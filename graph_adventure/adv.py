...
def backtrack():
    ...
    while backtracking is True:
        if player.currentRoom.id not in open_branches and len(open_branches) > 0:
            player.travel(reversalPath[-1])
            traversalPath.append(reversalPath[-1])
            reversalPath.pop()
        elif len(open_branches) > 0:
            backtracking = False
        else:
            break
...
def traverse(back_dir=None, back_id=None):
    ...
    if not current in graph:
        ...
        for exit in player.currentRoom.getExits():
            graph[current][exit] = '?'
        graph[current][back_dir] = back_id
        if '?' in graph[current].values():
            open_branches.add(current)
    ...
    if current in open_branches:
        if 'n' in graph[current] and graph[current]['n'] == '?':
            previous = current
            traversalPath.append('n')
            reversalPath.append('s')
            player.travel('n')
            graph[previous]['n'] = player.currentRoom.id
            if '?' not in graph[previous].values():
                open_branches.remove(previous)
            traverse('s', previous)
        elif 'e' in graph[current] and graph[current]['e'] == '?':
            previous = current
            traversalPath.append('e')
            reversalPath.append('w')
            player.travel('e')
            graph[previous]['e'] = player.currentRoom.id
            if '?' not in graph[previous].values():
                open_branches.remove(previous)
            traverse('w', previous)
        elif 's' in graph[current] and graph[current]['s'] == '?':
            previous = current
            traversalPath.append('s')
            reversalPath.append('n')
            player.travel('s')
            graph[previous]['s'] = player.currentRoom.id
            if '?' not in graph[previous].values():
                open_branches.remove(previous)
            traverse('n', previous)
        elif 'w' in graph[current] and graph[current]['w'] == '?':
            previous = current
            traversalPath.append('w')
            reversalPath.append('e')
            player.travel('w')
            graph[previous]['w'] = player.currentRoom.id
            if '?' not in graph[previous].values():
                open_branches.remove(previous)
            traverse('e', previous)
    elif current not in open_branches and len(open_branches) > 0:
        backtrack()
        traverse()
